...
void DECOMP_UI_DrawRaceClock(u_short paramX, u_short paramY, u_int flags, struct Driver* driver)
{
	...
	#else
	DECOMP_DecalFont_DrawLine("TIME:", (int)(short)paramX, (int)(short)paramY+16, fontType, (int)strFlags_but_its_also_posY);	
	#endif
	...
	//+ value in X to adjust it to be after lap string
	DECOMP_DecalFont_DrawLine(displayTime, posX+50, numParamY >> 0x10, FONT_SMALL, (int)strFlags_but_its_also_posY);
	#else
