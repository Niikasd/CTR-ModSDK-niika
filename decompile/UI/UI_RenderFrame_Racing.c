	...
	if (playerThread != 0)
	{
		...
		do
		{
			...
			if ((gameMode1 & BATTLE_MODE) == 0)
			{
				...
				{
					//X,Y
					DECOMP_UI_DrawLapCount(172, 200, (u_int)hudStructPtr[1].scale, playerStruct);
				}
				#endif
				#ifndef USE_ONLINE
				if ((playerStruct->actionsFlagSet & 0x2000000) == 0)
...
// not rewritten yet
			...
			if ((partTimeVariable5 & 0x4020020) == 0)
			{
				...
				pos[1] = '\0';
				DECOMP_DecalFont_DrawLine(&pos, 170, 192, FONT_BIG, ORANGE);
				DECOMP_DecalFont_DrawLine(sdata->lngStrings[data.stringIndexSuffix[playerStruct->driverRank]], 185, 200, FONT_SMALL, ORANGE);
	...
	if (numPlyr == 1)
	{
		...
		DECOMP_UI_DrawRaceClock(0x10, 184, 0, playerStruct);
